        ...
        for n_iter in range(1, max_iter + 1):
            ...
            for j in range(k):
                ...
                if type_name not in ['LinearTerm', 'ConstantTerm']:
                    ...
                    # Prepare arguments for golden_section
                    golden_section_args = {
                        "a": term_instance.lower_bound,
                        "c": term_instance.upper_bound,
                        "delta": 0.382,
                        "function": aic_func,
                        "tol": 1e-2,
                        "max_iter": 50,
                        "int_score": term_instance.int_score
                    }

                    # Conditionally add bw_max if mgwr version is 2.2.1 or later
                    if self.mgwr_version >= pkg_resources.parse_version("2.2.1"):
                        golden_args["bw_max"] = 1e8 

                    # Run golden_section with dynamically set arguments
                    sig, aic, _ = golden_section(**golden_section_args)    
